...
char *vi_Version =
	"$Id: virus.c,v 0.0.6 2003/09/08 13:07:21 ripclaw Exp $";
...
/* this was in here from busybox, will sort out later FIXME
 * Things To Do:
 *	EXINIT
 *	$HOME/.exrc  and  ./.exrc
...
#endif							/* BB_FEATURE_VI_CRASHME */
	status_buffer = (Byte *) malloc(BUFSIZ_STATBUF);	// hold messages to user
#ifdef BB_FEATURE_VI_READONLY
	...
	if (strchr("iIaAsc", cm)) {	// multi-char commands
		...
		for (i = 0; i < cnt; i++) {
			if (thing == 0) {	// insert chars
				...
				strlcat((char *) readbuffer, multilines[(int) lrand48() % 20], BUFSIZ); /* FIXED added BUFSIZ and converted to stlcat() */
				sleeptime = 0;	// how fast to type
			}
		}
...
static void print_literal(Byte * buf, Byte * s) // copy s to buf, convert unprintable
{
	...
	for (; *s > '\0'; s++) {
		c = *s;
		if (*s > '~') {
			strlcat((char *) buf, SOs, BUFSIZ); /* FIXED strlcat BUFSIZ */
			c = *s - 128;
		}
		if (*s < ' ') {
			strcat((char *) buf, "^");
		...
		b[0] = c;
		strlcat((char *) buf, (char *) b, BUFSIZ); /* FIXED added BUFSIZ and converted to strlcat() */
		if (*s > '~')
			strlcat((char *) buf, SOn, BUFSIZ); /* FIXED strlcat BUFSIZ */
		if (*s == '\n') {
			strcat((char *) buf, "$");
		}
	}
...
static Byte get_one_char()
{
	...
	if (!adding2q) {
		...
		if (last_modifying_cmd != 0) {
			...
			if (len + 1 >= BUFSIZ) {
				/* FIXME last_modifying_cmd needs a shaving */
				fprintf(stderr,"last_modifying_cmd overrun");
				exit(EXIT_FAILURE);
			} else {
				// add new char to q
...
static void place_cursor(int row, int col, int opti)
{
	...
	/* FIXME - possible replace with strlc* ? */

	// move to the correct row
	while (row < Rrow) {
		// the cursor has to move up
		strlcat(cm2, CMup, BUFSIZ); /* FIXED strlcat BUFSIZ */
		Rrow--;
	}
	while (row > Rrow) {
		// the cursor has to move down
		strlcat(cm2, CMdown, BUFSIZ); /* FIXED strlcat BUFSIZE */
		Rrow++;
	}
...
static void psbs(char *format, ...)
{
	...
	/* FIXED strcpy((char *) status_buffer, SOs); */
	strlcpy((char *) status_buffer, SOs, BUFSIZ_STATBUF);	// Terminal standout mode on
	vsprintf((char *) status_buffer + strlen((char *) status_buffer), format,
			 args);
	/* FIXED strcat((char *) status_buffer, SOn); */
	strlcat((char *) status_buffer, SOn, BUFSIZ_STATBUF);	// Terminal standout mode off
	va_end(args);
