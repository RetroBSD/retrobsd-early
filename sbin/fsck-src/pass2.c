...
again:
		...
		case FCLEAR:
			...
			setstate(dirp->d_ino, DIRCT(dp) ? DSTATE : FSTATE);
			goto again;
		...
		case DSTATE:
			...
			if (getstate(dirp->d_ino) == DFOUND) {
				declncnt(dirp->d_ino);
			} else if (getstate(dirp->d_ino) == DCLEAR) {
				dirp->d_ino = 0;
				ret |= ALTERED;
			} else
