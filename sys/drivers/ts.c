...
struct	ts_softc {
	...
	u_int	sc_uba;		/* Unibus addr of cmd pkt for tsdb */
...
#define	INF		((daddr_t) ((u_int) 65535))
...
u_int softspace[NTS][(sizeof(struct ts_softc)/2) + 1];
...
register int unit;
{
	register u_int sp = (u_int) softspace[unit];
	...
	if (((u_int)&sc->sc_cmd  - (ubmap ? (u_int)softspace : 0)) & 3)
		sp += 2;
	if (ubmap && TS_ubadr == 0) {
	...
	sc->sc_addr = addr;
	sc->sc_uadr = TS_ubadr + ((u_int)&sc->sc_cmd -
				(ubmap ? (u_int)softspace : 0));
...
loop:
	...
	if ((blkno = sc->sc_blkno) == dbtofsb(bp->b_blkno)) {
		um->b_active = SIO;
		tc->c_loba = (u_int) bp->b_un.b_addr;
		tc->c_hiba = bp->b_xmem;
...
tsinit(ts11)
{
	...
	if (sc->sc_addr->tssr & (TS_NBA | TS_OFL)) {
		...
		if (cnt >= 10000)
			return (1);
		tchar->char_bptr = (u_int) loint(sc->sc_uadr)+
			((u_int) &sc->sc_sts-(u_int)tcmd);
		...
		tcmd->c_loba = (u_int)loint(sc->sc_uadr)+
			((u_int)tchar-(u_int)tcmd);
