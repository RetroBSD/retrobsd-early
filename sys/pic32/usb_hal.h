...
#include <machine/usb_common.h>

#if defined(USB_SUPPORT_HOST) || defined(USB_SUPPORT_OTG)

#if defined(__C30__)
	#include <machine/usb_hal_pic24.h>
#endif
...
int USBHALControlBusPower (unsigned char cmd);
...
/*************************************************************************
    Function:
        int USBHALStallPipe(TRANSFER_FLAGS pipe)
...
/******************************************************************************
    Function:
        int USBHALUnstallPipe(TRANSFER_FLAGS pipe)
...
unsigned short USBHALGetStalledEndpoints (void);


/******************************************************************************
    Function:
        int USBHALFlushPipe(TRANSFER_FLAGS pipe)
...
int USBHALTransferData (TRANSFER_FLAGS    flags,
                          void             *buffer,
                          unsigned int      size     );
...
int USBHALSetEpConfiguration (unsigned char ep_num, unsigned short max_pkt_size, unsigned short flags);
...
int USBHALInitialize (unsigned long flags);
