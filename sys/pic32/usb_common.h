...
// Section: Return values for usb_host_device_status()

#define USB_DEVICE_STATUS                       0x30                        // Offset for usb_host_device_status() return codes
...
/* Data Transfer Flags
 *
 * The following flags are used in the flags parameter of the "usbdev_transfer_data"
 * and "usbhal_transfer_data" routines.  They can be accessed by the bitfield
...
typedef enum
{
    ...
    // responsible for monitoring Vbus and calling usb_host_vbus_event() with this
    ...
    // usb_host_vbus_event() with this event.  This event is not generated by the
...
typedef struct _transfer_event_data
{
    ...
    unsigned int    size;	// Actual number of bytes transferred
...
/*
 *   Function:
 *       int <Event-handling Function Name> (USB_EVENT event,
 ...
 *
 *   Preconditions:
 *       usb_initialize must have been called to initialize the USB SW
 *       Stack.
 *
 *   Paramters:
 ...
 */
typedef int (*USB_EVENT_HANDLER) (USB_EVENT event, void *data, unsigned int size);
...
/*
 *   Function:
 *       int usb_initialize (unsigned long flags)
 *
 *   Summary:
 ...
 */
#ifndef usb_initialize
    #if defined(USB_SUPPORT_DEVICE)
        #if defined(USB_SUPPORT_HOST)
            #if defined(USB_SUPPORT_OTG)
                #error "USB OTG is not yet supported."
            #else
                #define usb_initialize(f) \
                        (usbdev_initialize(f) && usb_host_init(f)) ? \
                        1 : 0
            #endif
        #else
            #define usb_initialize(f) usbdev_initialize(f)
        #endif
    #else
        #if defined(USB_SUPPORT_HOST)
            #define usb_initialize(f) usb_host_init(f)
        #else
 ...
 *
 *   Precondition:
 *       usb_initialize() has been called.
 *
 *   Parameters:
 *       None
 *
 ...
 *       application or by default is defined correctly for the stack mode.
 */
#ifndef usb_tasks    // Implemented as a macro that can be overridden.
    #if defined(USB_SUPPORT_DEVICE)
        #if defined(USB_SUPPORT_HOST)
            #if defined(USB_SUPPORT_OTG)
                #error "USB OTG is not yet supported."
            #else
                #define usb_tasks() { usb_host_tasks(); usbhal_handle_bus_event(); }
            #endif
        #else
            #define usb_tasks() usbhal_handle_bus_event()
        #endif
    #else
        #if defined(USB_SUPPORT_HOST)
            #define usb_tasks() usb_host_tasks()
        #else
            #error "Application must define support mode"
        #endif
    #endif
