...
void mips64_check_cpu_pausing(mips64_jit_tcb_t *b)
{
  ...
  mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
  x86_mov_membase_imm(b->jit_ptr,X86_EDI,OFFSET(cpu_mips_t,state),CPU_STATE_PAUSING,4);
 mips64_jit_tcb_push_epilog(b);
 /*else do noting*/
 x86_patch(test1,b->jit_ptr);
...
/* BEQL (Branch On Equal Likely) */
 static int  mips_emit_beql(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)
{
   int rs = bits(insn,21,25);
   int rt = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
/* BGEZ (Branch On Greater or Equal Than Zero) */
 static int  mips_emit_bgez(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BGEZAL (Branch On Greater or Equal Than Zero And Link) */
 static int  mips_emit_bgezal(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BGEZALL (Branch On Greater or Equal Than Zero and Link Likely) */
 static int  mips_emit_bgezall(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BGEZL (Branch On Greater or Equal Than Zero Likely) */
 static int  mips_emit_bgezl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BGTZ (Branch On Greater Than Zero) */
 static int  mips_emit_bgtz(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1,*test2;
...
/* BGTZL (Branch On Greater Than Zero Likely) */
 static int  mips_emit_bgtzl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn) 
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1,*test2;
...
/* BLEZ (Branch On Less or Equal Than Zero) */
 static int  mips_emit_blez(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn) 
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1,*test2;
...
/* BLEZL (Branch On Less or Equal Than Zero Likely) */
 static int  mips_emit_blezl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1,*test2;
...
/* BLTZ (Branch On Less Than Zero) */
 static int  mips_emit_bltz(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BLTZAL (Branch On Less Than Zero And Link) */
 static int  mips_emit_bltzal(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BLTZALL (Branch On Less Than Zero And Link Likely) */
 static int  mips_emit_bltzall(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BLTZL (Branch On Less Than Zero Likely) */
 static int  mips_emit_bltzl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int offset = bits(insn,0,15);
   u_char *test1;
...
/* BNE (Branch On Not Equal) */
 static int  mips_emit_bne(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int rt = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
/* BNEL (Branch On Not Equal Likely) */
 static int  mips_emit_bnel(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs = bits(insn,21,25);
   int rt = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
/* BREAK */
 static int  mips_emit_break(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{	
...
/* CACHE */
 static int  mips_emit_cache(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{        
 ...
 static int  mips_emit_cfc0(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{        
...
static int  mips_emit_clz(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{  
	//x86_alu_reg_imm(b->jit_ptr,X86_SUB,X86_ESP,12);
   x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_cop0(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
...
static int  mips_emit_cop2(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dadd(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_daddi(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_daddiu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_daddu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_ddiv(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_ddivu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_dmtc0(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dmult(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dmultu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsll(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
static int  mips_emit_dsllv(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsrlv(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsrav(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsub(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsubu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsrl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsra(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsll32(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsrl32(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_dsra32(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_eret(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
...
/* J (Jump) */
static int  mips_emit_j(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
...
static int  mips_emit_jal(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	u_int instr_index = bits(insn,0,25);
...
static int  mips_emit_jalr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	int rs = bits(insn,21,25);
   int rd = bits(insn,11,15);
   m_va_t ret_pc;
...
static int  mips_emit_jr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	int rs = bits(insn,21,25);
...
static int  mips_emit_lb(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_lbu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_ld(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
  mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_ldc1(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
  mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_ldc2(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
  mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_ldl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
  mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_ldr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
  mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_lh(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_lhu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_ll(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_lld(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_lwc2(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_lwl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_lwr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_lwu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base   = bits(insn,21,25);
   int rt     = bits(insn,16,20);
   int offset = bits(insn,0,15);
...
static int  mips_emit_mad(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
...
static int  mips_emit_madd(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	//x86_alu_reg_imm(b->jit_ptr,X86_SUB,X86_ESP,12);
   x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_maddu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	//x86_alu_reg_imm(b->jit_ptr,X86_SUB,X86_ESP,12);
   x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_mfc0(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_msub(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	//x86_alu_reg_imm(b->jit_ptr,X86_SUB,X86_ESP,12);
   x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_msubu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	//x86_alu_reg_imm(b->jit_ptr,X86_SUB,X86_ESP,12);
   x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_mtc0(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_sb(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base = bits(insn, 21, 25);
   int rt = bits(insn, 16, 20);
   int offset = bits(insn, 0, 15);
...
static int  mips_emit_sc(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base = bits(insn, 21, 25);
   int rt = bits(insn, 16, 20);
   int offset = bits(insn, 0, 15);
...
static int  mips_emit_scd(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_sd(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_sdc2(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_sdl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_sdr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_sh(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base = bits(insn, 21, 25);
   int rt = bits(insn, 16, 20);
   int offset = bits(insn, 0, 15);
...
static int  mips_emit_swc2(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_swl(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base = bits(insn, 21, 25);
   int rt = bits(insn, 16, 20);
   int offset = bits(insn, 0, 15);
...
static int  mips_emit_swr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int base = bits(insn, 21, 25);
   int rt = bits(insn, 16, 20);
   int offset = bits(insn, 0, 15);
...
static int  mips_emit_sync(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	return (0);
}
static int  mips_emit_syscall(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
...
static int  mips_emit_teq(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
  int rs = bits(insn,21,25);
   int rt = bits(insn,16,20);
   u_char *test1;
...
static int  mips_emit_teqi(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
   int rs  = bits(insn,21,25);
   int imm = bits(insn,0,15);
   m_reg_t val = sign_extend(imm,16);
...
static int  mips_emit_tlb(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	 uint16_t func = bits(insn, 0, 5);
   return mips_tlb_jit[func].emit_func(cpu, b,insn);
}

static int  mips_emit_tlbp(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_tlbr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_tlbwi(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_tlbwr(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_set_pc(b,b->start_pc+((b->mips_trans_pos-1)<<2));
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_tge(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_tgei(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_tgeu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_tgeiu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_tlt(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_tlti(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
static int  mips_emit_tltiu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
static int  mips_emit_tltu(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_tne(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	x86_mov_reg_imm(b->jit_ptr,X86_EDX,insn);
   x86_mov_reg_reg(b->jit_ptr,X86_EAX,X86_EDI,4);
...
static int  mips_emit_tnei(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
static int  mips_emit_wait(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	return (0);
}
...
static int  mips_emit_unknownBcond(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_unknowncop0(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_unknownmad(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
...
static int  mips_emit_unknownSpec(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}

static int  mips_emit_unknowntlb(cpu_mips_t *cpu,mips64_jit_tcb_t *b,unsigned int insn)  
{
	mips64_emit_unknown(cpu,b,insn);
   return(0);
}
