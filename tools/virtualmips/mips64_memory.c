...
static void *mips_mts64_access (cpu_mips_t * cpu, m_va_t vaddr,
    ...
    u_int is_fromgdb);
static int mips_mts64_translate (cpu_mips_t * cpu, m_va_t vaddr,
 ...
 */
static no_inline mts64_entry_t *mips_mts64_map (cpu_mips_t * cpu,
    u_int op_type, mts_map_t * map, mts64_entry_t * entry,
...
static void *mips_mts64_lookup (cpu_mips_t * cpu, m_va_t vaddr)
{
    ...
    return (mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LOOKUP, 4, MTS_READ,
...
u_int mips_mts64_gdb_lb (cpu_mips_t * cpu, m_va_t vaddr, void *cur)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LB, 1, MTS_READ,
...
u_int mips_mts64_lb (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LB, 1, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
...
u_int mips_mts64_lbu (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LBU, 1, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_lh (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LH, 2, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_lhu (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LHU, 2, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_lw (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LW, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_lwu (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LWU, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_ld (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LD, 8, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_sb (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_SB, 1, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_sh (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_SH, 2, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
...
u_int mips_mts64_sw (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_SW, 4, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_sd (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    data = cpu->gpr[reg];
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_SD, 8, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_lwl (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x03);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_LWL, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_lwr (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x03);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_LWR, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_ldl (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x07);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_LDL, 8, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_ldr (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x07);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_LDR, 8, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_swl (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x03ULL);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SWL, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
    ...
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SWL, 4, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if ((haddr == NULL) && (has_set_value == FALSE)) {
        bad_memory_access (cpu, vaddr);
    }
...
u_int mips_mts64_swr (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x03);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SWR, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
    ...
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SWR, 4, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if ((haddr == NULL) && (has_set_value == FALSE)) {
        bad_memory_access (cpu, vaddr);
    }
...
u_int mips_mts64_sdl (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x07);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SDL, 8, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
    ...
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SDL, 8, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if ((haddr == NULL) && (has_set_value == FALSE)) {
        bad_memory_access (cpu, vaddr);
    }
...
u_int mips_mts64_sdr (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    naddr = vaddr & ~(0x07);
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SDR, 8, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
    ...
    haddr =
        mips_mts64_access (cpu, naddr, MIPS_MEMOP_SDR, 8, MTS_WRITE, &data,
        &exc, &has_set_value, 0);
    if ((haddr == NULL) && (has_set_value == FALSE)) {
        bad_memory_access (cpu, vaddr);
    }
...
u_int mips_mts64_ll (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    haddr =
        mips_mts64_access (cpu, vaddr, MIPS_MEMOP_LL, 4, MTS_READ, &data,
        &exc, &has_set_value, 0);
    if (exc)
        return exc;
    if ((haddr == NULL) && (has_set_value == FALSE)) {
...
u_int mips_mts64_sc (cpu_mips_t * cpu, m_va_t vaddr, u_int reg)
{
    ...
    if (cpu->ll_bit) {
        data = cpu->gpr[reg] & 0xffffffff;
        haddr =
            mips_mts64_access (cpu, vaddr, MIPS_MEMOP_SC, 4, MTS_WRITE, &data,
            &exc, &has_set_value, 0);
        if (exc)
            return exc;
        if ((haddr == NULL) && (has_set_value == FALSE)) {
...
void mips_mts64_api_map (cpu_mips_t * cpu, m_va_t vaddr, m_pa_t paddr,
    m_uint32_t len, int cache_access, int tlb_index)
{
    /* nothing to do, the cache will be filled on-the-fly */
}

void mips_mts64_api_unmap (cpu_mips_t * cpu, m_va_t vaddr, m_uint32_t len,
...
/* MTS64 slow lookup */
static mts64_entry_t *mips_mts64_slow_lookup (cpu_mips_t * cpu,
    m_uint64_t vaddr, u_int op_code, u_int op_size, u_int op_type,
    m_reg_t * data, u_int * exc, mts64_entry_t * alt_entry, u_int is_fromgdb)
{
    ...
    case 0xffffff:
        ...
        case 0x7fc:            /* ckseg0 */
            ...
            if (!(entry =
                    mips_mts64_map (cpu, op_type, &map, entry, alt_entry)))
                goto err_undef;
  ...
  err_address:
    if (is_fromgdb)
        return NULL;
    mips_access_special (cpu, vaddr, MTS_ACC_AE, op_code, op_type, op_size,
        data, exc);
    return NULL;
  err_tlb:
    if (is_fromgdb)
    ...
    u_int is_fromgdb)
{
    ...
    *exc = 0;
    ...
    if (unlikely (((vaddr & MIPS_MIN_PAGE_MASK) != entry->gvpa) || cow)) {
        entry =
            mips_mts64_slow_lookup (cpu, vaddr, op_code, op_size, op_type,
            data, exc, &alt_entry, 0);
        if (!entry)
            return NULL;
    ...
    m_uint32_t * phys_page)
{
    ...
    if (unlikely ((vaddr & MIPS_MIN_PAGE_MASK) != entry->gvpa)) {
        entry =
            mips_mts64_slow_lookup (cpu, vaddr, MIPS_MEMOP_LOOKUP, 4,
...
*/
    ...
    *exc = 0;
    ...
    if (unlikely (mips_mts32_check_tlbcache (cpu, vaddr, op_type,
                entry) == 0)) {
        entry =
            mips_mts32_slow_lookup (cpu, vaddr, op_code, op_size, op_type,
