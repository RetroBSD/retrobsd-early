...
virtual_breakpoint_t *alloc_breakpoint(vm_instance_t * vm, m_uint32_t addr, char *save, int len)
{
...
static void attach_gdb(vm_instance_t * vm)
{
   ...
   gdb_fd = accept(vm->gdb_listen_sock, (struct sockaddr *) &sockaddr1, &tmp);
   ...
   tmp = 1;
   setsockopt(gdb_fd, SOL_SOCKET, SO_KEEPALIVE, (char *) &tmp, sizeof(tmp));

   tmp = 1;
   setsockopt(gdb_fd, IPPROTO_TCP, TCP_NODELAY, (char *) &tmp, sizeof(tmp));
      ...
      case 'q':
         {
            ...
            else if (strncmp(cmd_buf, QOFFSETS, strlen(QOFFSETS)) == 0)
            {
      ...
      case 'Z':
         {
            ...
            if ('0' == (cmd_buf + 1)[0])
            {
               ...
               else
               {
                  ...
                  virtual_breakpoint_t *tmp = alloc_breakpoint(vm, addr, 0, len);
