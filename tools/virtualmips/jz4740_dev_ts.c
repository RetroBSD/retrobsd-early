...
void dev_jz4740_ts_cb(void *opaque)
{
   ...
   if ((jz4740_ts_table[SADC_ENA / 4] & SADC_ENA_TSEN))
   {
      if ((jz4740_ts_table[SADC_CTRL / 4] & SADC_CTRL_PENDM) == 0)
      {
         ...
         if (ts_ev != NULL)
         {
            d->x = ((ts_ev->button.x) * (TSMAXX - TSMINX) / d->ds->width) + TSMINX;
            d->y = ((ts_ev->button.y) * (TSMAXY - TSMINY) / d->ds->height) + TSMINY;
            if (d->x > TSMAXX)
               d->x = TSMAXX;
            if (d->y > TSMAXY)
               d->y = TSMAXY;
                           ...
                           m_uint32_t offset, u_int op_size, u_int op_type, m_reg_t * data, m_uint8_t * has_set_value)
{
   ...
   case SADC_CFG:
      if (op_type == MTS_WRITE)
      {
         ASSERT((*data & SADC_CFG_TS_DMA) == 0x0, "Only touche screen model is support in SADC \n");
