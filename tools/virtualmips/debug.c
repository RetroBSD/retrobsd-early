...
void vm_debug_init(vm_instance_t * vm)
{
   ...
   tmp = 1;
   if (setsockopt(vm->gdb_listen_sock, SOL_SOCKET, SO_REUSEADDR, (char *) &tmp, sizeof(tmp)) < 0)
   {
   ...
   while (bind(vm->gdb_listen_sock, (struct sockaddr *) &sockaddr, sizeof(sockaddr)) || listen(vm->gdb_listen_sock, 1))
   {
      vm->gdb_port++;
      sockaddr.sin_port = htons(vm->gdb_port);
   }
