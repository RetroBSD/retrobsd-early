...
static int vtty_tcp_conn_wait(vtty_t * vtty)
{
   ...
   if (setsockopt(vtty->accept_fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) < 0)
   {
...
vtty_t *vtty_create(vm_instance_t * vm, char *name, int type, int tcp_port, const vtty_serial_option_t * option)
{
   vtty_t *vtty;
...
void vtty_put_char(vtty_t * vtty, char ch)
{
   ...
   case VTTY_TYPE_TCP:
      if ((vtty->state == VTTY_STATE_TCP_RUNNING) && (fwrite(&ch, 1, 1, vtty->fstream) != 1))
      {
